...
def validate_data(age, investment_amount, intent_request):
    ...
    # Validate age is between 0 and 65 years old
    if age is not None:
        age=parse_int(age) # make sure age is integer
        if age not in range(0,65):
            return build_validation_result(
                False,
                "age",
                "You should be above 0 years old and younger than 65 to use this service, "
                "please provide a different age.",
            )

    # Validate the investment amount, it should be > 0
    if investment_amount is not None:
        investment_amount = parse_int(investment_amount)  # make sure investment amout is integer
        if investment_amount < 5000:
            return build_validation_result(
                False,
                "investmentAmount",
                "The amount to invest be greater than USD 5000, "
                "please provide an amount greater than USD 5000.",
            )
...
def recommend_portfolio(intent_request):
    ...
    # get invenstment recommendation
    
    invest_recommendation = risk(risk_level)

        
        
    # Return a message with conversation's result.
    return close(
        intent_request["sessionAttributes"],
        "Fulfilled",
        {
            ...
            we recommend the following portfolio allocations based on your input {}
            """.format(
                first_name,invest_recommendation
            ),
        },
    )
